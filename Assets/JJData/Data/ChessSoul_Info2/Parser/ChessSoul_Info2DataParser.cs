...
public static class ChessSoul_Info2DataParser
{
    ...
    {
        int stringLength = 0;
        TextAsset binAsset = Resources.Load<TextAsset>(Path.Combine(_dataFilePath, "ChessSoul_Info2/ChessSoul"));
        Span<byte> buffer = binAsset.bytes;
        int offset = 0;
        long rows = TypeByte2TypeConverter.ConvertTypeByte2long(buffer.Slice(offset, 8));
        offset += 8;
        ChessSoul_Info2.ChessSoul = new ChessSoul_Info2_ChessSoul[rows];
        for(int row = 0; row < rows; row++)
        {
            ChessSoul_Info2_ChessSoul sheetRowData = new();
            ...
            offset += 12;
            ChessSoul_Info2.ChessSoul[row] = sheetRowData;
        }
